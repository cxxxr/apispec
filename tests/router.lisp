...
(deftest find-route-tests
  (let* ((get-pet-operation (make-instance 'operation
         ...
         (paths `(("/pets/{petId}" . ,specified-pet)
                  ("/pets/me" . ,my-pet)))
         (router (make-router paths)))
    (ok (eq (find-route router :get "/pets/me")
            get-my-pet-operation))
    (ok (eq (find-route router :get "/pets/14")
            get-pet-operation))
    (ok (null (find-route router :post "/pets/me")))
    (ok (null (find-route router :get "/pets")))))
